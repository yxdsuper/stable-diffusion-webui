...
class ScriptPostprocessingUpscale(scripts_postprocessing.ScriptPostprocessing):
    ...
    def ui(self):
        ...
        with gr.Tabs(elem_id="extras_resize_mode"):
            with gr.TabItem('按比例缩放', elem_id="extras_scale_by_tab") as tab_scale_by:
                upscaling_resize = gr.Slider(minimum=1.0, maximum=8.0, step=0.05, label="调整大小", value=4,
                                             elem_id="extras_upscaling_resize")

            with gr.TabItem('按尺寸缩放', elem_id="extras_scale_to_tab") as tab_scale_to:
                with FormRow():
                    ...
                    upscaling_crop = gr.Checkbox(label='裁剪以适应尺寸', value=True, elem_id="extras_upscaling_crop")

        with FormRow():
            extras_upscaler_1 = gr.Dropdown(label='缩放公式 1', elem_id="extras_upscaler_1",
                                            choices=[x.name for x in shared.sd_upscalers],
                                            value=shared.sd_upscalers[0].name)

        with FormRow(visible=False):
            extras_upscaler_2 = gr.Dropdown(label='缩放公式 2', elem_id="extras_upscaler_2",
                                            choices=[x.name for x in shared.sd_upscalers],
                                            value=shared.sd_upscalers[0].name)
