...
def create_seed_inputs(target_interface):
    with FormRow(elem_id=target_interface + '_seed_row'):
        seed = (gr.Textbox if cmd_opts.use_textbox_seed else gr.Number)(label='种子编号', value=-1,
        ...
        with gr.Group(elem_id=target_interface + '_subseed_show_box'):
            seed_checkbox = gr.Checkbox(label='Extra', visible=False, elem_id=target_interface + '_subseed_show',
                                        value=False)
                ...
                with gr.Column(scale=80):
                    with gr.Row():
                        negative_prompt = gr.Textbox(label="Negative prompt", elem_id=f"{id_part}_neg_prompt",
                                                     ...
                                                     placeholder="在此输入你不希望图片中出现的内容")

        button_interrogate = None
        button_deepbooru = None
        if is_img2img:
            with gr.Column(scale=1, elem_id="interrogate_col"):
                button_interrogate = gr.Button('图片描述', elem_id="interrogate")
                button_deepbooru = gr.Button('反推提示词', elem_id="deepbooru")

        with gr.Column(scale=1, elem_id=f"{id_part}_actions_column"):
            with gr.Row(elem_id=f"{id_part}_generate_box"):
                interrupt = gr.Button('中断', elem_id=f"{id_part}_interrupt")
                skip = gr.Button('跳过', elem_id=f"{id_part}_skip")
                submit = gr.Button('开始生成', elem_id=f"{id_part}_generate", variant='primary')
            ...
            with gr.Row(elem_id=f"{id_part}_styles_row"):
                prompt_styles = gr.Dropdown(label="我的词库", elem_id=f"{id_part}_styles",
...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            sampler_index = gr.Dropdown(label='渲染方法', elem_id=f"{tabname}_sampling",
                                        choices=[x.name for x in choices], value=choices[0].name, type="index")
            steps = gr.Slider(minimum=1, maximum=150, step=1, elem_id=f"{tabname}_steps", label="渲染步数",
                              value=20)
    else:
        with FormGroup(elem_id=f"sampler_selection_{tabname}"):
            steps = gr.Slider(minimum=1, maximum=150, step=1, elem_id=f"{tabname}_steps", label="渲染步数",
                              value=20)
            sampler_index = gr.Radio(label='渲染方法', elem_id=f"{tabname}_sampling",
                                     choices=[x.name for x in choices], value=choices[0].name, type="index")
...
def create_ui():
                    ...
                    if category == "sampler":
                        ...
                        with FormRow():
                            with gr.Column(elem_id="txt2img_column_size", scale=4):
                                width = gr.Slider(minimum=64, maximum=2048, step=8, label="图像宽度", value=512,
                                                  elem_id="txt2img_width")
                                height = gr.Slider(minimum=64, maximum=2048, step=8, label="图像高度", value=512,
                                                   elem_id="txt2img_height")

                            res_switch_btn = ToolButton(value=switch_values_symbol, elem_id="txt2img_res_switch_btn")
                            if opts.dimensions_and_batch_together:
                                with gr.Column(elem_id="txt2img_column_batch"):
                                    batch_count = gr.Slider(minimum=1, step=1, label='生成次数', value=1,
                                                            elem_id="txt2img_batch_count")
                                    batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='生成基数', value=1,
                                                           elem_id="txt2img_batch_size")

                    elif category == "cfg":
                        cfg_scale = gr.Slider(minimum=1.0, maximum=30.0, step=0.5, label='描述词影响度', value=7.0,
        ...
        with FormRow().style(equal_height=False):
            with gr.Column(variant='compact', elem_id="img2img_settings"):
                ...
                def add_copy_image_controls(tab_name, elem):
                    with gr.Row(variant="compact", visible=False, elem_id=f"img2img_copy_to_{tab_name}"):
                        # gr.HTML("Copy image to: ", elem_id=f"img2img_label_copy_to_{tab_name}")

                        for title, name in zip(['img2img'
                                                # , 'sketch', 'inpaint', 'inpaint sketch'
                                                ],
                                               ['img2img'
                                                # , 'sketch', 'inpaint', 'inpaint_sketch'
                                                ]):
                            if name == tab_name:
                ...
                with gr.Tabs(elem_id="mode_img2img"):
                    with gr.TabItem('图像变换', id='img2img', elem_id="img2img_img2img_tab") as tab_img2img:
                    ...
                    # with gr.TabItem('Sketch', id='img2img_sketch', visible=False,
                    #                 elem_id="img2img_img2img_sketch_tab") as tab_sketch:
                ...
                with FormRow():
                    resize_mode = gr.Radio(label="模型尺寸", elem_id="resize_mode",
                                           choices=["拉伸", "裁切",
                                                    # "Resize and fill",
                    ...
                    if category == "sampler":
                        ...
                        with FormRow():
                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                width = gr.Slider(minimum=64, maximum=2048, step=8, label="图片宽度", value=512,
                                                  elem_id="img2img_width")
                                height = gr.Slider(minimum=64, maximum=2048, step=8, label="图片高度", value=512,
                        ...
                        with FormRow(elem_id="img2img_checkboxes", variant="compact", visible=False):
                            restore_faces = gr.Checkbox(label='Restore faces', value=False,
                                                        visible=len(shared.face_restorers) > 1,
                        ...
                        with FormGroup(elem_id="inpaint_controls", visible=False) as inpaint_controls:
                            ...
                            for i, elem in enumerate(
                                    [tab_img2img,
                                     # tab_sketch,
                                     # tab_inpaint, tab_inpaint_color, tab_inpaint_upload,
                                     # tab_batch
                                     ]):
                                elem.select(
    ...
    interfaces = [
        (txt2img_interface, "描述图像", "txt2img"),
        (img2img_interface, "图像变换", "img2img"),
        (extras_interface, "放大图片", "extras"),
